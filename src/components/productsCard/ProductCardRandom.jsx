...
const ProductCardRandom = React.memo(({ product }) => {
  const {
    state: { cart },
    dispatch,
  } = CartState();
  ...
  return (
    <div className="w-[240px] border-solid border-2 border-slate-800 rounded-2xl flex flex-col md:mb-0 mb-4">
      ...
      <div className="mb-2">
        ...
        <p className="font-semibold">{`${formatarPreco(product.preco)}`}</p>
        <div className="flex justify-center">
          ...
          {typeof product.preco === "number" && (
            <p>Até 10 x de {`${formatarPreco(product.preco / 10)}`}</p>
          )}
        </div>
      </div>
      {alertMessage ? (
        ...
        <Button
          onClick={addToCart}
          className="w-1/2 mx-auto mb-4 bg-black text-white"
        >
          + Carrinho
        </Button>
      )}
